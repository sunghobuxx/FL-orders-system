...
- **실거래 데이터로 시험하지 않는다.** 단위 테스트는 가짜 db 객체로 하고, DB 를 실제로
  건드려야 하는 검증은 **테스트 식당으로만** 한다.
  `organization_id = af6378e4-0000-0000-0000-000000000001`
  `restaurant_id   = 7306bb64-4596-4a5e-92ab-6e9dd9395238` (정산주기 monthly)
  다른 업체의 정산서·명세서·미수금은 어떤 단계에서도 만들거나 고치지 않는다.
- 운영 DB 는 Supabase 프로젝트 `nvpaggacvbotgqyxfdof` (ap-northeast-2) 하나뿐이다.
  로컬 dev 서버도 같은 DB 를 본다. 그래서 위 규칙이 로컬에서도 그대로 적용된다.
...
```sql
select count(*) as 팔월확정건수 from sales_statements
where confirmed_at is not null and created_at >= '2026-08-01';
```

Expected: `0`. Task 1 백필은 완납된 과거분만 채우므로 8월 진행분은 확정되지 않아야 한다.
0 이 아니면 백필 조건이 잘못된 것이니 멈추고 확인한다.
...
- [ ] **Step 1: 검증 전제를 갖춘다 — 사용자에게 받을 것**

테스트 식당은 지금 상태로는 두 가지를 검증할 수 없다. 시작 전에 사용자에게 확인한다.

1. **연락처가 없다** (`contacts` 1건이지만 `phone` 이 null). 문자가 실제로 도달하는지
   보려면 번호가 필요하다. **사용자 본인 번호를 받아서** 넣는다. 다른 사람 번호를
   쓰면 모르는 사람에게 문자가 간다.
2. **정산주기가 monthly** 라 일정산 제외를 검증할 수 없다. 검증할 때만 잠시 daily 로
   바꾸고 끝나면 되돌린다.
...
```sql
-- 원본을 먼저 확인한다
select id, name, phone, is_primary from contacts
where organization_id = 'af6378e4-0000-0000-0000-000000000001';

update contacts set phone = '<사용자가 준 번호>'
where organization_id = 'af6378e4-0000-0000-0000-000000000001';
```

- [ ] **Step 2: 사용자에게 push 허가를 받고 배포한다**

`git push` 는 사용자가 지시할 때만 한다. 배포는 GitHub push → CF 자동배포뿐이다.
`gh run list --workflow=deploy.yml --limit 1` 로 완료를 확인한다.

- [ ] **Step 3: 검증용 정산서를 고르고 원본을 기록한다**

테스트 식당의 마감된 정산서 중 하나를 고른다:

```sql
select s.id, sp.start_date, sp.end_date, s.total_amount, s.outstanding_amount,
       s.confirmed_at, s.confirmed_total, s.notified_at
from sales_statements s
join settlement_periods sp on sp.id = s.settlement_period_id
where s.restaurant_id = '7306bb64-4596-4a5e-92ab-6e9dd9395238'
order by sp.end_date desc;
```

고른 정산서 id 와 위 값 전부를 스크래치패드 파일에 적어 둔다. 되돌릴 때 쓴다.

- [ ] **Step 4: 확정한다**

운영 `/admin/settlement/confirm` 에서 그 한 건만 체크해 확정한다.
확정 후 확인:

```sql
select confirmed_at, confirmed_total, total_amount, notified_at
from sales_statements where id = '<대상>';
```

Expected: `confirmed_at` 채워짐, `confirmed_total = total_amount`

- [ ] **Step 5: 잠금이 걸리는지 확인한다 (검증 1·2·3)**

세 경로를 각각 시도하고 매번 금액이 그대로인지 본다.

1. **명세서를 붙인다** — 테스트 식당에 그 기간 날짜로 명세서를 하나 만든다
   (`/admin/settlement/specs` 또는 발주 후 동기화). 정산서 금액이 그대로이고
   그 명세서가 다음 기간 정산서에 담기는지 확인한다.
2. **단가를 재적용한다** — 테스트 식당이 쓰는 품목의 단가를 등록한다
   (`/admin/products`). `price-snapshots` 경로가 막히는지 본다.
3. **명세서 라인을 직접 고친다** — `/admin/settlement/specs/<specId>` 에서
   수량이나 단가를 바꾼다. `update-spec-line` 경로가 막히는지 본다.

각 시도 뒤 매번 확인한다:

```sql
select total_amount, confirmed_total,
       total_amount = confirmed_total as 금액유지
from sales_statements where id = '<대상>';
```

Expected: 세 번 모두 `금액유지 = true`

- [ ] **Step 6: 문자가 도달하는지 확인한다 (검증 4)**

Step 1 에서 넣은 번호로 문자가 왔는지 사용자에게 확인받는다.
알림톡 템플릿이 아직 없으면 폴백으로 문자가 나가야 한다.

```sql
select notified_at from sales_statements where id = '<대상>';
```

Expected: `notified_at` 채워짐

- [ ] **Step 7: 일정산 제외를 확인한다 (검증 8)**

테스트 식당을 잠시 일정산으로 바꾼다:

```sql
update restaurants set settlement_cycle = 'daily'
where id = '7306bb64-4596-4a5e-92ab-6e9dd9395238';
```

확정 화면을 새로 고쳐 다른 정산서 한 건을 확정한다 →
「발송 대상 아님(일정산)」으로 남고 문자가 안 나가는지 확인한다.

끝나면 **반드시 되돌린다**:

```sql
update restaurants set settlement_cycle = 'monthly'
where id = '7306bb64-4596-4a5e-92ab-6e9dd9395238';
```

- [ ] **Step 8: 연락처 없음을 확인한다 (검증 5)**
...
또 다른 정산서를 확정한다 → 확정은 되고 「연락처 없음」으로 남는지 확인한다.
끝나면 Step 1 의 번호로 되돌린다.

- [ ] **Step 9: 공개 링크를 확인한다 (검증 6)**

Step 4 에서 발급된 토큰을 찾는다:

```sql
select token, expires_at from statement_share_links where statement_id = '<대상>';
```

`https://order.fruitlife.shop/s/<token>` 을 **로그인 안 한 브라우저**로 연다 →
정산서가 보이는가.

유효기간을 지난 것으로 만들어 다시 연다:
...
- [ ] **Step 11: 검증에 바꾼 것을 전부 되돌린다**

Step 3 에 적어 둔 원본으로 되돌린다. 확정 자체도 되돌린다 —
테스트 식당이라도 확정 상태로 두면 다음 검증 때 헷갈린다.

```sql
update sales_statements
set confirmed_at = null, confirmed_by = null, confirmed_total = null, notified_at = null
where restaurant_id = '7306bb64-4596-4a5e-92ab-6e9dd9395238';

delete from statement_share_links
where statement_id in (
  select id from sales_statements where restaurant_id = '7306bb64-4596-4a5e-92ab-6e9dd9395238'
);
```
...
```sql
select
  (select count(*) from sales_statements
    where restaurant_id = '7306bb64-4596-4a5e-92ab-6e9dd9395238'
      and confirmed_at is not null) as 남은확정,
  (select settlement_cycle from restaurants
    where id = '7306bb64-4596-4a5e-92ab-6e9dd9395238') as 정산주기,
  (select count(*) from statement_share_links l
    join sales_statements s on s.id = l.statement_id
    where s.restaurant_id = '7306bb64-4596-4a5e-92ab-6e9dd9395238') as 남은링크;
```

Expected: 남은확정 0, 정산주기 `monthly`, 남은링크 0

- [ ] **Step 12: 검증 결과를 사용자에게 보고한다**

8개 검증 항목 각각의 결과를 사실대로 보고한다. 통과하지 못한 항목이 있으면 그대로 말한다.
Step 11 의 원복 확인 SQL 결과를 함께 보고한다.
